        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-sm text-gray-700 leading-5">
                    {!! __('全') !!}<span class="font-medium">{{ $paginator->total() }}</span>{!! __('件中') !!}
                    @if ($paginator->firstItem())
                        <span class="font-medium">{{ $paginator->firstItem() }}</span>{!! __('件〜') !!}
                        <span class="font-medium">{{ $paginator->lastItem() }}</span>{!! __('件 を表示') !!}
                    @else
                        {{ $paginator->count() }}
                    @endif
                </p>
